...
* 그래서 일단 사망 상태를 즉사와 빈사를 구분해 GA와 태그를 분리했다
  ...
  * [Input Mapping Context를 제어](https://jaykop.github.io/unreal/combat/PlayerRestriction/)하는 방법으로 플레이어의 입력을 제한했다
...
* 전용 Anim Layer를 만드는 것을 고민했는데, 마침 ALS가 Crouch를 지원하고 해당 프로젝트에서는 Crouch를 사용하지 않기에, 빈사 시 Animation적으로 Crouch 상태가 되도록 했다
