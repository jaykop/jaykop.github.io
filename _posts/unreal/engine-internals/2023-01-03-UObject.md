...
## [Class Default Object](https://jaykop.github.io/unreal/engine-internals/CDO/)
* 각 UCLASS는 CDO라는 하나의 오브젝트 객체를 유지
* CDO는 생성자에 의해 생성된 이후, 수정되지 않는 탬플릿 용도의 오브젝트
* Read-Only지만, GetClass() 메서드를 통해 접근 가능

## [Garbage Collector](https://jaykop.github.io/unreal/engine-internals/UnrealGarbageCollection/)
    ...
    * UObjectBaseUtility
      * UObject
        ...
        * UField부터는 리플렉션 데이터 관련 객체이다
          * UEnum - CPP 열거형을 위한 리플렉션 테이터 클래스
          * FProperty - CPP 멤버 변수, 함수 인자를 위한 리플렉션 데이터 클래스
          * UStruct
            * UFunction - CPP 함수를 위한 리플렉션 데이터 클래스
            * UScriptStruct - CPP 구조체를 위한 리플렉션 데이터 클래스
            * UClass - CPP 클래스를 위한 리플렉션 데이터 클래스
...
![post_thumbnail](/assets/images/UObject/01.png)
