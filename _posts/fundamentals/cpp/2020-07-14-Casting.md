...
// constless object를 read만 하고, write을 하지 않기 때문
...
```c++
// ex1
Parent* parent1 = new Parent; // Parent 그 자체
Child* child = new Child; // Child 그 자체
Parent* parent2 = new Child; // Parent의 탈을 쓴 Child

Parent* upcasted = dynamic_cast<Parent*>(child);    // ok
Child* downcasted1 = dynamic_cast<Child*>(parent2); // ok

Child* downcasted2 = dynamic_cast<Child*>(parent1); // failed
```
...
* **다형 형식의 파생 클래스 간의 캐스팅**
  * virtual method로 다형 형식의 계층이어야만 [RTTI](https://jaykop.github.io/fundamentals/cpp/rtti/)를 사용해 캐스팅 가능
