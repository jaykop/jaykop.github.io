...
![post_thumbnail](/assets/images/MemoryAllocator/01.png)

* 메모리를 하나의 청크로 선 할당
  * 이를 다시 여러 블록으로 나눠
  * **freelist** - 나누어진 블록으로 이루어진 링크리스트
* 할당 시, 사용자는 하나 이상의 메모리에 엑세스를 얻음
  * 메모리 블록에 대한 포인터
  * 이 블록은 사용 중으로 마킹
* 반환 시, 유저가 메모리 매니저에게 직접 반환
  * 이 블록은 가용 가능한 것으로 다시 마킹
...
![post_thumbnail](/assets/images/MemoryAllocator/02.png)

* 메모리 관리에 있어 가장 큰 문제
* 파편화로 인해 가용 가능한 메모리 범위를 벗어나는 경우가 많음

### Stack Allocator
![post_thumbnail](/assets/images/MemoryAllocator/03.png)
...
* 다양한 사이즈의 Pool을 이용할 수는 있음
  * [Segregated Fit](https://jaykop.github.io/fundamentals/cpp/MemoryAllocator/#segregated-fit)
...
### First Fit
![post_thumbnail](/assets/images/MemoryAllocator/04.png)
...
![post_thumbnail](/assets/images/MemoryAllocator/05.png)
...
## Word
* 하드웨어 또는 프로세서에 의해 제어되는, 고정 크기의 데이터 처리 단위
...
![post_thumbnail](/assets/images/MemoryAllocator/06.png)
...
```
![post_thumbnail](/assets/images/MemoryAllocator/07.png)
...
```
![post_thumbnail](/assets/images/MemoryAllocator/08.png)
...
```
![post_thumbnail](/assets/images/MemoryAllocator/09.png)
