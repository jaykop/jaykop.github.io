...
// 아래와 같이 선언했을 때, 
MyClass<int, int> m;    // MyClass<T, T>와 MyClass<T, int> 간의 모호성으로 인한 에러 발생
MyClass<int*, int*> m;  // MyClass<T, T>와 MyClass<T1*, T2*> 간의 모호성으로 인한 에러 발생
...
### 템플릿 함수의 부분 특수화
* 기본적으로 불가능하다
* 템플릿 클래스의 멤버 함수 전체를 특수화할 수는 있으나, 일부만 특수화는 불가능하다
* 함수 템플릿 자체도 부분 특수화는 불가능하다
...
### 암시적 구체화
* 일반적으로 템플릿 함수를 구현하고 이를 사용할 때 발생
...
int main()
{
  // T = int로 하는 템플릿 함수의 구현이 필요하다고 암시적으로 컴파일러에게 전달
  add(10, 5);
}
```

### 명시적 구체화
* 템플릿 함수의 정의를 이용하여 특정 타입을 사용하는 함수를 정의하라고 컴파일러에게 명시적으로 전달
* 이를 전달받은 컴파일러는 해당 함수의 사용 여부에 상관없이 정의 코드를 생성한다
  * 원래는 템플릿 함수를 정의하고, 이를 특정 타입에 대해 사용하는 경우에만 컴파일러가 코드를 생성한다
...
int main()
{
  // T가 double 인지 int 인지 애매함 -> 컴파일 에러 발생
  add(10, 5.5);

  // T = double로 하는 템플릿 함수의 구현이 필요하다고 명시적으로 컴파일러에게 전달
  add<double>(10, 5);
}
```
