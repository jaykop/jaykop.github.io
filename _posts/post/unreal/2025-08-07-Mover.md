...
2. Authority
  ...
  * 서버 시뮬레이션으로 일치 여부 확인 (Detect Errors)
    * 결과가 다르면 즉시 서버에서 결정한 상태 정보로 수정 (Replicate State)
3. Peer Client / Proxy
...
1. Automonous Client
  ...
  * 서버로는 입력 정보만 전달
2. Authority
  * 클라이언트에서 받은 입력 정보 버퍼에 저장
  * 입력 키 바탕으로 시뮬레이션 해서 캐릭터의 위치 결정
  * 위치 및 상태 정보와 함께 입력 정보도 같이 클라이언트에 제공
3. 서버로부터 받은 입력 정보로 시뮬레이션 해 상태 및 위치 결장
