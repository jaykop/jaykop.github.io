...
### 정의
* 실체화 또는 일급이라고 하면 어떤 개념을 데이터(객체)로 바꿀 수 있다는 의미이다
  * 이 개념은 값일 수도, 함수와 같은 로직일 수도 있다
* **명령 패턴이란, 메서드 호출을 실체화한 것이며, 함수 호출을 객체로 감쌌다는 의미**이다
  * 콜백, 일급 함수, 함수 포인터, 클로저 등의 용어가 이에 해당한다

### 예시 - 입력 키 변경 기능

![image](/assets/images/DesignPattern/Command_1.png)
...
// 실제 입력 처리 함수
// 이제 각 버튼 입력 시, execute 함수를 호출하기만 하면 된다
void InputHandler::handlelnput() {
...
* 위의 코드에서 게임 내 모든 액터를 제어할 수 있다
  * AI 코드에서도 액터를 Command로 제어하는 것이 가능하다
...
* 특정 함수 호출을 위해 Class를 만들고, 그 Class의 객체를 통해 호출하는 방식으로 바뀌었다
  * 즉, 함수(데이터/로직)의 객체화가 이뤄졌다
...
* 명령을 실행할 객체를 함수로부터 반환받아 인자로 넘기거나, 다른 인자를 받아 실행시킬 수도 있다